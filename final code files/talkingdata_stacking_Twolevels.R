...
##For boruta+rf model and for xgboost
testset_rf=testset
testset_xg=testset
...
keepimp=keepimp[,-c(1)]
length(keepimp)
final_trainset=trainset_rf[,names(trainset_rf) %in% keepimp]
dim(final_trainset)
...
write.csv(res_submit,"E:\\AbhinavB\\Kaggle\\TalkingData\\SubmissionFiles\\Submit8_TWOlevel_stacking.csv",row.names = F,quote = F)
