...
#trainset$group=as.factor(trainset$group)
str(trainset)
tr_labels=trainset[,"group"]
train_m=trainset[,-c(1)]
##train_m=sparse.model.matrix(group ~ .-1,data=trainset)
cat.namevec=c("group","nummodel","numbrand")
...
#testset$group=as.factor(testset$group)
ts_labels=testset[,"group"]
test_m=testset[,-c(1)]
sparse_test_m=as.matrix(test_m)

#lgbtest=lgb.Dataset(data=as.matrix(test_m),label=ts_labels)
...
pred=predict(lgbmodel1,sparse_test_m)
...
write.csv(res_submit,"E:\\AbhinavB\\Kaggle\\TalkingData\\SubmissionFiles\\lightgbm1.csv",row.names = F,quote = F)
