...
##for boruta +rf model and xgboost 
trainset_rf=trainset
trainset_xg=trainset
...
##For boruta+rf model amd for xgboost
testset_rf=testset
testset_xg=testset
...
keepimp=keepimp[,-c(1)]
final_trainset=trainset_rf[,c("group",keepimp)]
...
pred_detail=matrix(pred_xg,nrow = length(ts_labels))
res_submit_xg=as.data.frame(pred_detail)
...
write.csv(res_submit,"E:\\AbhinavB\\Kaggle\\TalkingData\\SubmissionFiles\\Submit6_weightedAverage.csv",row.names = F,quote = F)
